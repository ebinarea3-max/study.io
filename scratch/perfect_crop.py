...
for i, box in enumerate(sorted_boxes):
    ...
    cx, cy = x + w//2, y + h//2
    # USE MASSIVE PADDING so the glow doesn't get cut off!
    size = max(w, h)
    size = int(size * 1.6) # 60% padding!
    half = size // 2
    ...
    alpha = cv2.pow(alpha / 255.0, 0.7) * 255
    ...
    # Upscale by 3x right here for clarity!
    upscaled = cv2.resize(bgra, (bgra.shape[1]*3, bgra.shape[0]*3), interpolation=cv2.INTER_LANCZOS4)
