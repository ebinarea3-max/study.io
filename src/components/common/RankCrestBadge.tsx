...
export function RankCrestBadge({
  ...
  const renderComposition = () => {
    const iconSize = typeof size === 'number' ? size * scale * 0.45 : '45%';
    const largeIconSize = typeof size === 'number' ? size * scale * 0.55 : '55%';
      ...
      case 'Platinum':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center pt-2">
            <Feather size={iconSize} stroke={accentLight} strokeWidth={2.5} className="absolute -translate-y-4 translate-x-2 rotate-12 drop-shadow-md" />
            <BookOpen size={largeIconSize} stroke={accentLight} strokeWidth={2} className="absolute translate-y-2 drop-shadow-md" />
          </div>
        );
      case 'Diamond':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center pt-1">
            <Brain size={iconSize} stroke={accentLight} strokeWidth={2.5} className="absolute -translate-y-4 drop-shadow-[0_0_15px_rgba(56,189,248,0.6)]" />
            <BookOpen size={largeIconSize} stroke={accentLight} strokeWidth={2} className="absolute translate-y-4 drop-shadow-md" />
          </div>
        );
      case 'Champion':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <Trophy size={largeIconSize} stroke={accentLight} strokeWidth={2.5} className="absolute drop-shadow-[0_0_20px_rgba(239,68,68,0.6)]" />
            <Sparkles size={iconSize} stroke={steelLight} strokeWidth={2} className="absolute -translate-y-8 translate-x-8 scale-75 animate-pulse" />
          </div>
        );
      case 'Master':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <Crown size={iconSize} stroke={accentLight} strokeWidth={2.5} className="absolute -translate-y-5 drop-shadow-[0_0_20px_rgba(245,158,11,0.6)]" />
            <BookOpen size={largeIconSize} stroke={accentLight} strokeWidth={2} className="absolute translate-y-3 drop-shadow-md" />
          </div>
        );
      case 'Grandmaster':
        return (
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <div className="absolute inset-0 rounded-full border-4 border-dashed border-orange-500/30 animate-[spin_10s_linear_infinite]" />
            <div className="absolute inset-2 rounded-full border-2 border-amber-400/50 animate-[spin_7s_linear_infinite_reverse]" />
            <Crown size={iconSize} stroke={accentLight} strokeWidth={3} className="absolute -translate-y-6 drop-shadow-[0_0_25px_rgba(234,88,12,0.8)]" />
            <BookOpen size={largeIconSize} stroke={accentLight} strokeWidth={2.5} className="absolute translate-y-4 drop-shadow-[0_0_15px_rgba(245,158,11,0.5)]" />
            <Sparkles size={iconSize} stroke="#FFFBEB" strokeWidth={2} className="absolute -translate-y-8 -translate-x-8 scale-50 animate-pulse" />
            <Sparkles size={iconSize} stroke="#FFFBEB" strokeWidth={2} className="absolute translate-y-8 translate-x-8 scale-50 animate-pulse delay-150" />
          </div>
        );
      default:
        return null;
