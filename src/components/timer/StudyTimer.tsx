    ...
    <div className="w-full max-w-6xl mx-auto h-[calc(100dvh-4rem)] md:h-auto flex flex-col md:grid md:grid-cols-1 lg:grid-cols-12 justify-between md:justify-start gap-4 lg:gap-6 overflow-hidden md:overflow-visible p-4 md:p-0">
      ...
      <div className="lg:col-span-8 flex-1 md:flex-initial flex flex-col justify-between overflow-hidden md:overflow-visible space-y-0 md:space-y-6">
        ...
        <div className="relative rounded-3xl bg-neutral-900/50 border border-white/[0.08] hover:border-emerald-500/25 backdrop-blur-2xl p-3.5 sm:p-8 lg:p-10 shadow-2xl overflow-hidden flex-1 md:flex-initial flex flex-col justify-between transition-all">
          ...
          {/* On-Screen Realtime Debug Line (Prompt Section 1 for mobile phone verification) */}
          <div className="w-full my-2 px-3 py-2 rounded-xl bg-slate-900/90 border border-slate-700/60 text-[11px] font-mono flex items-center justify-between gap-2 shadow-inner">
            <div className="flex items-center gap-1.5 overflow-hidden text-ellipsis whitespace-nowrap">
              ...
              <span className="text-slate-400 text-[10px] truncate max-w-[120px] sm:max-w-[180px]" title={realtimeUserId || 'None'}>
                UID: {realtimeUserId ? realtimeUserId.slice(0, 8) + '...' : <span className="text-amber-400">Sign in to sync</span>}
              </span>
              <span className="text-slate-600">|</span>
              <span className="text-slate-400 text-[10px]">
                Events: <strong className="text-white">{realtimeEventsCount}</strong>
              </span>
            </div>
            <button
              onClick={() => refetchActiveSession()}
              className="px-2.5 py-1 rounded text-[10px] font-semibold bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/30 transition-all active:scale-95 cursor-pointer flex-shrink-0"
              title="Force fetch session from Supabase"
            >
              Sync Now
            </button>
          </div>
